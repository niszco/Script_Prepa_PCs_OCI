...
if (-not $enablingAdministrator) {
        Write-Host "Le compte 'Administrateur' est deja actif !"
}
...
function clientCodeRequest {
        ...
        Write-Host "Le mot de passe pour l'administrateur a ete modifie avec succes !"
}
...
function localAccountDeletion {
        ...
        if (-not $deletingLocalAccount) {
                Write-Host "Le compte local 'admin' n'a pas pu etre trouve et/ou supprime !"
        }
}
